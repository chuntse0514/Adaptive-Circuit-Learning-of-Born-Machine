...
def operator_pool(n_qubit):
    ...
    # for i in reversed(range(2, n_qubit)):
    #     pool.append(partial(PauliStringRotation, pauliString=('X' * i + 'Y', list(range(i+1)))))
    #     pool.append(partial(PauliStringRotation, pauliString=('Z' * i + 'Y', list(range(i+1)))))
    #     gate_description.append(f'e^[X0:{i-1} Y{i}]')
...
class adapt_DDQCL:
    ...
    def __init__(self, 
        ...
        self.params = nn.ParameterDict({
            'ry': nn.Parameter(torch.full((self.n_qubit,), np.pi/2), requires_grad=False),
        ...
        Criterion = {
            ...
            'Quantum relative entropy': lambda p, q: 1 - torch.sum(torch.sqrt(p * q)) ** 2,
            'MSE': lambda p, q: torch.sum(((p - q) * self.normalize_const) ** 2) / (2 ** self.n_qubit),
            'Probability alignment': lambda p, q: -torch.cosine_similarity(p, q, dim=0)
        }
    ...
    def fit(self):
        ...
        for i_epoch in range(self.n_epoch):
            ...
            max_grads, max_grad_indicies, max_grad_gates = self.select_operator()
            ...
            model = qml.QNode(circuit, dev, interface='torch', diff_method='backprop')
            num_freqs = {
                'ry': torch.ones_like(self.params['ry']),
                'append': torch.ones_like(self.params['append'])
            }
