...
def aclbm_circuit(ry_params, append_params, op_id_tuple, pool_gates, n_qubit):
    for q in range(n_qubit):
        qml.RY(ry_params[q], wires=q)
    # Using a Python loop over a concrete tuple of IDs
    for i, id_ in enumerate(op_id_tuple):
        pool_gates[id_](append_params[i])
    return qml.probs(wires=range(n_qubit))

def eval_circuit(ry_params, append_params, op_id_tuple, eval_params, pool_gates, n_qubit):
    for q in range(n_qubit):
        qml.RY(ry_params[q], wires=q)
    for i, id_ in enumerate(op_id_tuple):
        pool_gates[id_](append_params[i])
    for i in range(len(pool_gates)):
        pool_gates[i](eval_params[i])
    return qml.probs(wires=range(n_qubit))
...
class ACLBM(BaseModel):
    def __init__(self, data_class, n_epoch: int, n_iter: int, No: int, alpha: float, reduction_rate=None):
        ...
        self.filename = f'{self.image_dir}/ACLBM/ACLBM(data={data_class.name}, No={self.No}, t1={self.threshold1}, t2={self.threshold2}{suffix}).pdf'
    ...
    def select_operator(self, randomize=False):
        qnode = qml.QNode(eval_circuit, self.dev, interface='jax')
        def loss_fn(eval_p):
            prob = qnode(self.params['Ry-layer'], self.params['Append'], tuple(self.operatorID), eval_p, self.pool, self.n_qubit)
            return kl_loss(self.target_prob, prob)
    ...
    def fit(self, save=True):
        qnode = qml.QNode(aclbm_circuit, self.dev, interface='jax')
        for i_iter in range(self.n_iter):
            ...
            def train_step(params, opt_state, op_id_tuple):
                def loss_fn(p):
                    prob = qnode(p['Ry-layer'], p['Append'], op_id_tuple, self.pool, self.n_qubit)
                    return kl_loss(self.target_prob, prob), prob
            ...
            while True:
                ...
                if grad_norm < self.threshold2 or len(self.loss_history) >= self.n_epoch:
                    break
